...
export default function HomePage() {
  ...
  const toggleAttendance = async (studentId) => {
    ...
    try {
      ...
      showFlashError(e.message || 'Failed to update')
    } finally {
      setMarkingId(null)
    }
  }
  ...
  return (
    <div className="fade-in">
      <div
        className="mb-5 px-4 py-4 sm:mb-6 sm:px-6 sm:py-5 relative overflow-hidden"
      ...
      >
        ...
        <div className="relative z-10 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <span
            className="inline-flex w-fit font-condensed font-bold text-xs tracking-widest uppercase px-3 py-1.5"
            style={{
      ...
      {isOffDay && (
        <div
          className="mb-4 sm:mb-5 px-4 py-3 text-sm font-medium"
        ...
        >
          Off day. Attendance cannot be marked. The list will rotate on the next working day.
        </div>
      )}
      ...
      {students.length > 0 && !isOffDay && (
        <div
          className="mb-4 sm:mb-5 px-4 py-3 flex items-start gap-3"
        ...
        >
          ...
          <div className="min-w-0">
            ...
            <div className="text-xs" style={{ color: 'var(--muted)' }}>
              First slot today. Rotated from the previous working day.
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', overflow: 'hidden' }}>
        <div
          ...
          style={{
            gridTemplateColumns: isAdmin ? '48px 1fr 90px 110px 52px' : '48px 1fr 90px 110px',
            background: '#0d1014',
            ...
            >
              <div className="flex items-start justify-between gap-3 sm:contents">
                ...
                <div className="min-w-0 flex-1 sm:block">
                  <div className="text-sm sm:text-sm" style={{ color: 'var(--text)', fontWeight: isFirst ? 700 : 400 }}>
                    {student.name}
                  </div>
              ...
              <div className="hidden sm:block text-sm" style={{ color: 'var(--text)', fontWeight: isFirst ? 700 : 400 }}>
                {student.name}
              </div>
              <div className="hidden sm:block font-condensed text-xs tracking-wider uppercase" style={{ color: 'var(--muted)' }}>
                {student.studentId}
              </div>
...
function StatChip({ label, value, color }) {
  return (
    <div
      className="flex items-center gap-2 px-3 py-2 font-condensed font-bold text-xs tracking-widest uppercase"
